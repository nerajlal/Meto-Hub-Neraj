...
<head>
    ...
    <title>GoSlot Store - Verify OTP</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .otp-inputs input { width: 48px; height: 56px; text-align: center; font-size: 1.5rem; font-weight: 600; padding: 0; border: 1px solid var(--line); border-radius: var(--radius-sm); background: var(--bg); color: var(--dark); outline: none; transition: border-color 0.25s var(--ease), box-shadow 0.25s var(--ease); }
        .otp-inputs input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(46,125,50,0.15); }
        ...
        .btn-resend {
            background: none; border: none; padding: 0; font-size: 0.88rem; font-family: inherit; font-weight: 600; color: var(--primary); cursor: pointer; transition: color 0.2s;
        }
        .btn-resend:hover {
...
<body>
    ...
    <section class="login-section">
        <div class="login-card">
            <div class="login-header">
                <h1>Verify OTP</h1>
                <p>We've sent a 6-digit code to <strong>{{ $email ?? session('reset_email') }}</strong>. Please enter it below.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.otp.verify') }}" id="otpForm">
                ...
                <div class="otp-inputs">
                    <input type="text" maxlength="1" onkeyup="moveToNext(this, event)" autofocus>
            ...
            <form method="POST" action="{{ route('password.request') }}" class="text-center" style="margin-top: 15px;">
                @csrf
                <input type="hidden" name="email" value="{{ $email ?? session('reset_email') }}">
    ...
    <script>
        ...
        document.getElementById('otpForm').addEventListener('submit', function(e) {
            ...
            document.getElementById('otp_hidden').value = otpValue;
        });
    </script>
</body>
</html>
