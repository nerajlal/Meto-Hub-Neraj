...
<div class="collection-header" style="margin-bottom: 2rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: flex-end; gap: 1rem; flex-wrap: wrap;">
    <div style="max-width: 800px;">
        <h1 class="collection-title" style="font-size: 2rem; font-weight: 800; color: var(--primary-color); line-height: 1.2;">My Wishlist</h1>
        <p style="color: var(--text-muted); margin-top: 0.5rem; font-size: 1rem;">Products you've saved to buy later. Click heart to remove.</p>
    </div>
    ...
    @if($wishlists->count() > 0)
        <div class="product-grid">
            @foreach($wishlists as $wishlist)
                @if($wishlist->product)
                    @include('template_3.partials.product_card', ['product' => $wishlist->product])
                @endif
            @endforeach
        </div>
    @else
