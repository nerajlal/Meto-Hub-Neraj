...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            {{--
            <a href="javascript:void(0)" class="btn btn-outline-info shadow-sm disabled"
                style="pointer-events: none; opacity: 0.5;" title="Pool is disabled">
                <i class="fas fa-swimming-pool me-1"></i> Pool
            </a>
            --}}
            <button type="button" class="btn btn-outline-primary shadow-sm" data-bs-toggle="modal"
                data-bs-target="#packOfModal">
        ...
        <div class="modal-dialog">
            <form action="{{ route('admin.bundles.pack-of') }}" method="POST" class="modal-content border-0 shadow">
                ...
                <div class="modal-body px-4 pb-4">
                    <p class="text-muted small mb-4">Quickly create bundles for multiple units of the same product variant.
                    </p>
                    ...
                    <div class="row g-3">
                        <div class="col-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="quantity" class="form-control border-start-0" value="2" min="2"
                                    required>
                            </div>
                        </div>
                        <div class="col-6">
                            ...
                            <div class="input-group">
                                <span
                                    class="input-group-text bg-light border-end-0 small">{{ $currentTenant->currency ?? '₹' }}</span>
                                <input type="number" name="pack_price" class="form-control border-start-0"
    ...
    <div class="card border shadow-sm">
        <div class="card-header bg-white border-bottom-0 p-0">
            <ul class="nav nav-tabs px-3 pt-2">
                <li class="nav-item">
                    <a class="nav-link {{ !request('type') || request('type') == 'bundle' ? 'active fw-bold text-dark border-bottom-0' : 'text-muted' }}"
                        href="{{ route('admin.bundles', array_merge(request()->query(), ['type' => 'bundle', 'page' => 1])) }}">
                        Bundles
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request('type') == 'pack' ? 'active fw-bold text-dark border-bottom-0' : 'text-muted' }}"
                        href="{{ route('admin.bundles', array_merge(request()->query(), ['type' => 'pack', 'page' => 1])) }}">
                        Pack Of
                    </a>
                </li>
                <!-- <li class="nav-item">
        ...
        <div class="card-header bg-light border-bottom p-3">
            <div class="d-flex gap-3">
                    ...
                    <form action="{{ route('admin.bundles') }}" method="GET">
                        ...
                        <div class="input-group shadow-sm">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}"
                                ...
                                placeholder="Filter {{ request('type') == 'pack' ? 'packs' : 'bundles' }}">
                        </div>
                    </form>
                </div>
                ...
                <!-- Status Filter -->
                <div class="dropdown">
                    <button class="btn btn-white border shadow-sm text-secondary bg-white dropdown-toggle" type="button"
                        data-bs-toggle="dropdown">
                        <i class="fas fa-filter me-2"></i>
                        {{ request('status') ? ucfirst(request('status')) : 'All Status' }}
                    </button>
                    <ul class="dropdown-menu shadow-sm border-0">
                        <li><a class="dropdown-item small {{ !request('status') ? 'active bg-light text-dark fw-bold' : '' }}"
                                href="{{ route('admin.bundles', array_merge(request()->query(), ['status' => null, 'page' => 1])) }}">All
                                Status</a></li>
                        <li><a class="dropdown-item small {{ request('status') == 'active' ? 'active bg-light text-dark fw-bold' : '' }}"
                                href="{{ route('admin.bundles', array_merge(request()->query(), ['status' => 'active', 'page' => 1])) }}">Active</a>
                        </li>
                        <li><a class="dropdown-item small {{ request('status') == 'draft' ? 'active bg-light text-dark fw-bold' : '' }}"
                                href="{{ route('admin.bundles', array_merge(request()->query(), ['status' => 'draft', 'page' => 1])) }}">Draft</a>
                        </li>
                    </ul>
                </div>
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu shadow-sm border-0 dropdown-menu-end">
                        <li><a class="dropdown-item small {{ !request('sort') || request('sort') == 'newest' ? 'active bg-light text-dark fw-bold' : '' }}"
                                href="{{ route('admin.bundles', array_merge(request()->query(), ['sort' => 'newest', 'page' => 1])) }}">Newest
                                First</a></li>
                        <li><a class="dropdown-item small {{ request('sort') == 'oldest' ? 'active bg-light text-dark fw-bold' : '' }}"
                                href="{{ route('admin.bundles', array_merge(request()->query(), ['sort' => 'oldest', 'page' => 1])) }}">Oldest
                                First</a></li>
                        <li>
                            <hr class="dropdown-divider">
                        </li>
                        <li><a class="dropdown-item small {{ request('sort') == 'title_asc' ? 'active bg-light text-dark fw-bold' : '' }}"
                                href="{{ route('admin.bundles', array_merge(request()->query(), ['sort' => 'title_asc', 'page' => 1])) }}">Title
                                (A-Z)</a></li>
                        <li><a class="dropdown-item small {{ request('sort') == 'title_desc' ? 'active bg-light text-dark fw-bold' : '' }}"
                                href="{{ route('admin.bundles', array_merge(request()->query(), ['sort' => 'title_desc', 'page' => 1])) }}">Title
                                (Z-A)</a></li>
                    </ul>
                </div>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (packProductSelect) {
                packProductSelect.addEventListener('change', function () {
                    ...
                    const url = "{{ route('admin.products.variants', ['tenant' => request()->route('tenant'), 'id' => ':id']) }}".replace(':id', productId);
                    fetch(url)
