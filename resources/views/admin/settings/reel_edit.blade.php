...
<div class="container-fluid" style="max-width: 720px;">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
            <h1 class="h3 mb-0 text-primary">Edit Reel</h1>
            <p class="text-muted small mb-0">Update the reel details and linked product or collection.</p>
        </div>
    </div>

    <form action="{{ route('admin.settings.reels.update', $reel->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="url" name="instagram_url" id="instagram_url" class="form-control @error('instagram_url') is-invalid @enderror" 
                        placeholder="https://www.instagram.com/reel/ABC123..." value="{{ old('instagram_url', $reel->instagram_url) }}" required>
                    @error('instagram_url')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="title" id="title" class="form-control" placeholder="e.g. Summer Collection Showcase" value="{{ old('title', $reel->title) }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex gap-3">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="link_type" id="link_product" value="product" {{ old('link_type', $reel->link_type) === 'product' ? 'checked' : '' }} onchange="toggleLinkFields()">
                            <label class="form-check-label fw-semibold" for="link_product">
                                <i class="fas fa-box me-1 text-primary"></i> Product
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="link_type" id="link_collection" value="collection" {{ old('link_type', $reel->link_type) === 'collection' ? 'checked' : '' }} onchange="toggleLinkFields()">
                ...
                <div id="collection-field" class="mb-0" style="display: none;">
                    ...
                    <select name="collection_id" id="collection_id" class="form-select @error('collection_id') is-invalid @enderror">
                        ...
                        @foreach($collections as $collection)
                            <option value="{{ $collection->id }}" {{ old('collection_id', $reel->collection_id) == $collection->id ? 'selected' : '' }}>
                                {{ $collection->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="card border shadow-sm mb-4">
            <div class="card-body">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ old('status', $reel->status) ? 'checked' : '' }}>
